...
| `services/memory_service` — `write_calibration_event()` + `query_calibration_stats()` (Long-term Memory, confidence calibration) | ✅ Built and tested (mocked `asyncpg` pool — same real-Postgres caveat as `audit_service`). **Governance-mandated from Milestone 1** (ACCB accepted uncalibrated LLM confidence as a risk "on condition that the Long-term Memory calibration-tracking loop is actually built... from Milestone 1 onward, not treated as a nice-to-have" — `architecture-approval-report.md` §5), not a discretionary addition. Wired into `workflows/document_assessment`'s confirm-extraction node — every real assessment emits one calibration event per field automatically. **Needs a new table** (`long_term_memory_calibration`) not yet in `postgres-primary-init.sql` — DDL handed to the infra owner via `infrastructure/AGENTS.md` item 8 |
| OCR/document-classifier engine wiring (`tools/ocr`, `tools/documents`) | ❌ **Attempted, blocked** — neither the Tesseract binary nor `pytesseract` are available in this environment (same class of gap as Docker). Confirmed by direct check, not assumed; picking this back up needs either a different dev environment or a vendor/deployment decision first |
| Test suite | ✅ 181/181 passing (`uv run pytest tests/unit/mara`), `ruff check`/`format` clean |
