...
- **New agent** → `agents/<name>_agent/`, only if it passes the review board's test (Part 2 of the review): does it make a genuinely ambiguous judgment call, or does it transform already-decided input deterministically? The latter goes in `services/`. Its tool grant, autonomy level, confidence floor, and approval gate are declared in `shared/agent_profiles/profiles.py` — not as constants in the agent module ([../architecture/05-agent-architecture.md](../architecture/05-agent-architecture.md) §5.15).
- **New tool** → `tools/<category>/`, or `tools/mcp_servers/<name>/` if it's better isolated as a standalone MCP server (preferred for anything with its own runtime dependencies, per [../architecture/06-tool-architecture.md](../architecture/06-tool-architecture.md) §6.5). Its caller allow-list is *derived* from the agent profile registry (`ALLOWED_CALLERS = callers_allowed_for_tool(TOOL_NAME)`), never hand-written in the tool module — enforced by `tests/unit/mara/test_agent_profiles.py`, see §6.7.
